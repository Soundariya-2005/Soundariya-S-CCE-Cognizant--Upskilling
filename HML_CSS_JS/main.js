    ...
    const cardsData = eventsToRender.map(event => {
        ...
        return `
            <div class="eventCard" data-id="${event.id}">
                <img src="${event.image}" alt="${event.name}" title="${event.name}" class="gallery-img" ondblclick="this.classList.toggle('enlarged')" title="${event.name} – double-click to enlarge">
                <h3>${event.name}</h3>
